...
output_file=summary_ls_join2nodes.txt
echo Local Search: join2nodes >> output_file
...
do
    ...
    python main.py --alg ls --nh jc --step first --alpha "$alpha_prob" --beta "$beta_prob" -k "$k_value" "$file" >> $output_file
done
