...
output_file=summary_ls_move1node.txt
echo Local Search: move1node >> output_file
...
do
    ...
    python main.py --alg ls --nh m1 --step first --alpha "$alpha_prob" --beta "$beta_prob" -k "$k_value" "$file" >> $output_file
done
