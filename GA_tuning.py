    ...
    def mutate(self):
        ...
        for ind in range(self.n_solutions):
            ...
            if n_clusters > self.initial_n_clusters[ind]:
                ...
                #print(f"Joining probabilities:\n{join_prob}")
                #print(f"Cluster lengths:\n{cluster_lengths}")
                ...
                # Implement a mechanism to stop joins which lead to too large clusters (consider those with more than 40% of nodes)
                # This is done for efficiency reasons
                while sum(cluster_lengths[to_join_idx]) > 0.4*n_nodes:
                    del to_join_idx[random.randint(0, len(to_join_idx)-1)]
